...
_YF_TF: dict[str, tuple[str, str]] = {
    "1m":  ("1m",  "1d"),
    "5m":  ("5m",  "5d"),
    "10m": ("5m",  "5d"),   # resampled from 5m bars below
    "15m": ("15m", "5d"),
    "30m": ("30m", "5d"),
    "1h":  ("1h",  "30d"),
...
if _TV_OK:
    _TV_INTERVAL = {
        ...
        "10m": getattr(Interval, "INTERVAL_10_MINUTES", None),
        "15m": Interval.INTERVAL_15_MINUTES,
        "30m": getattr(Interval, "INTERVAL_30_MINUTES", None),
...
_CACHE_TTL: dict[str, float] = {
    "1m": 30, "5m": 45, "10m": 50, "15m": 60, "30m": 75,
...
def _yf_fetch(pair: str, tf: str) -> Optional[dict]:
    ...
    try:
        ...
        # yfinance does not expose every intraday interval for every symbol.
        # Build a real 10-minute close from 5-minute bars rather than
        # silently substituting a different timeframe.
        if tf == "10m" and interval == "5m":
            close_col = close_col.resample("10min").last().dropna()
            if volume_col is not None:
                volume_col = volume_col.resample("10min").sum().dropna()
        ...
        volume_ratio = 0.0
        if volume_col is not None and len(volume_col) >= 5:
            avg_volume = float(volume_col.tail(20).mean() or 0)
            if avg_volume > 0:
                volume_ratio = round(
                    float(volume_col.iloc[-1]) / avg_volume, 3
                )
...
ALL_TFS = ["1m", "5m", "10m", "15m", "30m", "1h", "4h", "1d", "1W"]
