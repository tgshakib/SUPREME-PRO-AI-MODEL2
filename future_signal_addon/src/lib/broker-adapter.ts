...
// ─── Non-Lag & Advanced Indicator Suite ───────────────────────────────────────

/** Weighted Moving Average (last value only) */
function wmaLast(arr: number[], period: number): number {
  const p = Math.min(period, arr.length);
  const sl = arr.slice(-p);
  let num = 0, den = 0;
  for (let i = 0; i < p; i++) { const wt = i + 1; num += sl[i]! * wt; den += wt; }
  return den === 0 ? (sl[sl.length - 1] ?? 0) : num / den;
}

/** Full WMA series (same length as input) */
function wmaSeriesAll(arr: number[], period: number): number[] {
  const out: number[] = [];
  for (let i = 1; i <= arr.length; i++) out.push(wmaLast(arr.slice(0, i), period));
  return out;
}

/**
 * Hull Moving Average — ~50% less lag than EMA.
 * HMA = WMA(2·WMA(n/2) − WMA(n), √n)
 */
function hullMALast(closes: number[], period: number): number {
  const p     = Math.min(period, closes.length);
  const half  = Math.max(2, Math.floor(p / 2));
  const sqrtn = Math.max(2, Math.round(Math.sqrt(p)));
  const halfS = wmaSeriesAll(closes, half);
  const fullS = wmaSeriesAll(closes, p);
  const diff  = halfS.map((v, i) => 2 * v - fullS[i]!);
  return wmaLast(diff, sqrtn);
}

/**
 * Zero-Lag EMA — removes ~half of standard EMA lag by adjusting input.
 * adj[i] = close[i] + (close[i] − close[i−lag])
 */
function zlemaLast(closes: number[], period: number): number {
  const p   = Math.min(period, closes.length);
  const lag = Math.max(1, Math.floor((p - 1) / 2));
  const adj = closes.map((v, i) => v + (v - (closes[Math.max(0, i - lag)] ?? v)));
  return emaLast(adj, p);
}

/**
 * ALMA (Arnaud Legoux MA) — Gaussian-weighted, near-zero lag.
 * offset=0.85 → high responsiveness; sigma=6 → balanced smoothness.
 */
function almaLast(closes: number[], period: number, offset = 0.85, sigma = 6): number {
  const p  = Math.min(period, closes.length);
...
/**
 * Laguerre RSI — fast, low-lag, low-whipsaw RSI variant (gamma ≈ 0.7).
 * Values > 70 = overbought, < 30 = oversold, moves faster than standard RSI.
 */
function laguerreRSI(closes: number[], gamma = 0.7): number {
  if (closes.length < 4) return 50;
  let l0 = closes[0]!, l1 = l0, l2 = l0, l3 = l0;
  for (const p of closes) {
    const [p0, p1, p2] = [l0, l1, l2];
  ...
  return (cu + cd) === 0 ? 50 : 100 * cu / (cu + cd);
}

/**
 * Fisher Transform — converts price to Gaussian distribution.
 * Sharp, decisive turning-point signals; crossings of the signal line are entries.
 */
function fisherTransformLast(closes: number[], period = 10): { fish: number; sig: number } {
  const p    = Math.min(period, closes.length);
  const hi   = Math.max(...closes.slice(-p));
  const lo   = Math.min(...closes.slice(-p));
  const hl   = hi - lo;
  const clmp = (v: number) => Math.min(0.999, Math.max(-0.999, v));
  const last = closes[closes.length - 1]!;
  const prev = closes[closes.length - 2] ?? last;
  const val  = hl === 0 ? 0 : clmp(2 * ((last - lo) / hl) - 1);
  const valP = hl === 0 ? 0 : clmp(2 * ((prev - lo) / hl) - 1);
  return {
    fish: 0.5 * Math.log((1 + val)  / (1 - val)),
    sig:  0.5 * Math.log((1 + valP) / (1 - valP)),
  };
}

/** CCI — Commodity Channel Index (>100 overbought, <−100 oversold) */
function cciLast(candles: Candle[], period = 14): number {
  const p    = Math.min(period, candles.length);
...
/** Williams %R — fast overbought/oversold (−20 = overbought, −80 = oversold) */
function williamsRLast(candles: Candle[], period = 14): number {
  const p  = Math.min(period, candles.length);
  const sl = candles.slice(-p);
...
function mfiLast(candles: Candle[], period = 14): number {
  ...
  for (let i = candles.length - p; i < candles.length; i++) {
    ...
    const tpp = (pr.high + pr.low + pr.close) / 3;
    const mf  = tp * (c.volume ?? 1000);
...
/**
 * Choppiness Index — 100 = maximum chop (random walk), ≈38 = strong trend.
 * CI > 61.8 = choppy/ranging; CI < 38.2 = strong trend.
 */
function choppinessIndex(candles: Candle[], period = 14): number {
  const p    = Math.min(period, candles.length);
  const sl   = candles.slice(-p);
  const atrS = sl.reduce((s, c) => s + (c.high - c.low), 0);
  const hi   = Math.max(...sl.map(c => c.high));
  const lo   = Math.min(...sl.map(c => c.low));
  const rng  = hi - lo;
  if (rng === 0 || atrS === 0) return 100;
  return Math.min(100, Math.max(0, 100 * Math.log10(atrS / rng) / Math.log10(p)));
}

/** Stochastic %K with %D smoothing */
function stochasticKD(candles: Candle[], period = 14, smooth = 3): { k: number; d: number } {
  const kVals: number[] = [];
  for (let i = Math.max(0, candles.length - smooth); i < candles.length; i++) {
    const sl = candles.slice(Math.max(0, i - period + 1), i + 1);
    const hi = Math.max(...sl.map(c => c.high));
    const lo = Math.min(...sl.map(c => c.low));
    kVals.push(hi === lo ? 50 : ((candles[i]!.close - lo) / (hi - lo)) * 100);
  }
  const k = kVals[kVals.length - 1] ?? 50;
  return { k, d: kVals.reduce((a, b) => a + b, 0) / Math.max(1, kVals.length) };
}

/**
 * Heiken Ashi — smoothed candle trend direction + consecutive streak count.
 * Higher streak = stronger confirmed move.
 */
function heikenAshiTrend(candles: Candle[]): { dir: "CALL" | "PUT"; streak: number } {
  if (candles.length === 0) return { dir: "CALL", streak: 0 };
  let haOpen  = (candles[0]!.open + candles[0]!.close) / 2;
  let haClose = (candles[0]!.open + candles[0]!.high + candles[0]!.low + candles[0]!.close) / 4;
  let dir: "CALL" | "PUT" = haClose >= haOpen ? "CALL" : "PUT";
  let streak = 1;
  for (let i = 1; i < candles.length; i++) {
    const c   = candles[i]!;
    const pO  = haOpen, pC = haClose;
    haOpen  = (pO + pC) / 2;
    haClose = (c.open + c.high + c.low + c.close) / 4;
    const nd: "CALL" | "PUT" = haClose >= haOpen ? "CALL" : "PUT";
    if (nd === dir) streak++; else { dir = nd; streak = 1; }
  }
  return { dir, streak };
}

// ─── Market Regime Classifier ──────────────────────────────────────────────────
// Prevents signals in flat/noisy conditions where TA is unreliable.

type MarketRegime = "strong_trend" | "weak_trend" | "ranging" | "choppy";

function classifyRegime(candles: Candle[]): { regime: MarketRegime; ci: number; adx: number } {
  const closes = candles.map(c => c.close);
  const ci     = choppinessIndex(candles, Math.min(14, candles.length));
  const adx    = trendStrength(closes, Math.min(14, closes.length - 1));
  const regime: MarketRegime =
    (ci <= 38.2 || adx >= 45) ? "strong_trend" :
    (ci <= 50   || adx >= 25) ? "weak_trend"   :
    (ci <= 61.8)              ? "ranging"       : "choppy";
  return { regime, ci, adx };
}

// ─── Signal Quality Interfaces ────────────────────────────────────────────────
...
export type SignalTier = "PREMIUM" | "STANDARD" | "SKIP";

export interface GodLevelSignal extends SignalQuality {
  /**
   * PREMIUM  — ≥ 88 score, ≥ 15/20 votes, ≥ 3 TF levels agree → direct win expected
   * STANDARD — ≥ 74 score, ≥ 11/20 votes, market not choppy   → high-probability win
   * SKIP     — low consensus / choppy / conflicting TFs         → no signal emitted
   */
  tier:   SignalTier;
  /** Composite 0–100 weighted score after multi-TF adjustment */
  score:  number;
