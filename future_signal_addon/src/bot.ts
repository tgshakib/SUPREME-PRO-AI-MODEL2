...
let integratedBot: Telegraf<MyContext> | null = null;
...
export async function startBot(): Promise<void> {
  ...
  await initAdapters();
  if (INTEGRATED_RELAY) {
    integratedBot = buildBot();
    if (!expiryWatcherStarted) {
      startExpiryWatcher(integratedBot);
      expiryWatcherStarted = true;
    }
    logger.info("Future Signal initialized in integrated relay mode; Telegram polling is disabled");
    return;
  }
...
export async function handleTelegramUpdate(update: unknown): Promise<void> {
  if (!INTEGRATED_RELAY) {
    throw new Error("Telegram update relay is disabled");
  }
  if (!integratedBot) {
    await startBot();
  }
  await integratedBot!.handleUpdate(update as never);
}
...
function buildBot(): Telegraf<MyContext> {
  ...
  bot.use(async (ctx, next) => {
    ...
    if (query && "data" in query && query.data.startsWith("tgadv:")) {
      query.data = query.data.slice("tgadv:".length);
    }
    if (
      INTEGRATED_RELAY
      && ctx.message
      && "text" in ctx.message
      && /^\/start(?:@\w+)?(?:\s|$)/i.test(ctx.message.text)
    ) {
      return;
    }
    return next();
  ...
  bot.command(["futuresignal", "tgfuturesignal"], async ctx => {
    const uid = ctx.from?.id ?? 0;
